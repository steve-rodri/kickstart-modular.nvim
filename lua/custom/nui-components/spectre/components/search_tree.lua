    ...
    return {
      {
        ...
        handler = function()
          ...
          if not has_children then
            ...
            local replacer = replacer_creator:new(spectre_state_utils.get_replace_engine_config(), replace_handler(tree, focused_node))
  ...
  if #node.diff.search > 0 then
    ...
    fn.ieach(node.diff.search, function(value, index)
      ...
      if replace_diff_value then
        local replace_text = string.sub(code_text, replace_diff_value[1] + 1 - empty_spaces, replace_diff_value[2] - empty_spaces)
